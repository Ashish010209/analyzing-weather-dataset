...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregated_value=df.pivot_table(index=df[date_col].dt.month, values=agg_col, aggfunc=agg)
...
def convert(df,celsius):
    ...
    def cal(r):
        p=32+(r/5)*9
        return p
    df[celsius]=cal(df[celsius])
    converted_temp=pd.DataFrame(df)
...
print(wind_speed_35_vis_25.head(3))
